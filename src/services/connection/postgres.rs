...
use sqlx::{
    ...
    ConnectOptions, Postgres, QueryBuilder, Row,
};
...
impl DatabasePlugin for PostgresPlugin {
    ...
    async fn connect(&self, config: &ConnectionConfig) -> anyhow::Result<ConnectionHandle> {
        let ConnectionConfig::Postgres(config) = config else {
            bail!("invalid config for postgres plugin");
        };
        ...
        if let Some(ssl_mode) = &config.ssl_mode {
            options = options.ssl_mode(match ssl_mode.as_str() {
                "disable" => PgSslMode::Disable,
                "prefer" => PgSslMode::Prefer,
                "require" => PgSslMode::Require,
                "verify_ca" => PgSslMode::VerifyCa,
                "verify_full" => PgSslMode::VerifyFull,
                _ => bail!("unsupported postgres ssl_mode: {ssl_mode}"),
            });
        }
    ...
    async fn metadata(&self, handle: &ConnectionHandle) -> anyhow::Result<DatabaseMetadata> {
        let ConnectionHandle::Postgres(pool) = handle else {
            bail!("invalid postgres connection handle");
        };
...
fn build_metadata(rows: Vec<sqlx::postgres::PgRow>) -> DatabaseMetadata {
